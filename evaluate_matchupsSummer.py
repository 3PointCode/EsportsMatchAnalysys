...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate model with matchups (team stats + champion counters).")
    ...
    p.add_argument("--split-value", default=None)
...
def main():
    ...
    counters = load_counters(args.counters_json)
    ...
    if y_true is not None:
        metrics = {
            ...
            "roc_auc": safe_auc(y_true, p_final),
            "brier": float(brier_score_loss(y_true, p_final)),
            "n_matches": int(len(y_true)),
        }

        print("=== Evaluation (with matchups) ===")
    ...
    out = matches.copy()
    out["p_blue_model"] = p_model
    out["p_blue_matchups"] = matchup_probs
    out["p_blue_final"] = p_final
